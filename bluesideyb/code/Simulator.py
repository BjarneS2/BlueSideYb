...
class Simulator:
    """
    mode='krylov' -> sparse Liouvillian + expm_multiply (good general default)
    mode='rk4_jit' -> Numba-JIT dense RK4 (very fast for a single small qubit if numba available)
    """
    def __init__(self, dt: float = 1e-7, mode: str = "krylov"):
        self.dt = float(dt)
        self.mode = mode
    ...
    def run(self,
        ...
        if tlist.ndim != 1 or len(tlist) < 2:
            raise ValueError("tlist must be 1D with at least two points")
        n_steps = len(tlist) - 1
        ...
        if self.mode == "krylov":
            ...
            # Cs_dense = [ _to_dense(C) for C in C_scaled_csr ]
            Hc_dense = _to_dense(Hc)
            Cs_dense = NumbaList.empty_list(types.complex128[:, :])
            ...
            for k in tqdm(range(n_steps), desc="Simulation progress"):
                ...
                H = Hc_dense + Htd
                rho = _rk4_step(H, Cs_dense, rho, dt)
                # enforce Hermiticity and trace = 1 numerically
...
def simulate(state: csr_matrix,
             ...
             mode: str = "krylov") -> Tuple[np.ndarray, List[csr_matrix]]:
    """
    ...
    return sim.run(state, c_hamiltonians, td_hamiltonians, c_ops, tlist, n_savepoints)
